...
// A linear trajectory that starts before the body's own trajectory.  The
// extremity is clamped where the body is first known; the collision interval
// is unaffected.
TEST_F(ApsidesTest_ComputeCollisionIntervals, TrajectoryBeforeReference) {
  DiscreteTrajectory<World> vessel_trajectory;
  AppendTrajectoryTimeline(
      NewLinearTrajectoryTimeline(
          DegreesOfFreedom<World>(
              World::origin +
                  Displacement<World>({-84 * Metre, 1 * Metre, 0 * Metre}),
              Velocity<World>({1 * Metre / Second,
                               0 * Metre / Second,
                               0 * Metre / Second})),
          /*Δt=*/1 * Second,
