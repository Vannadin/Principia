...
std::vector<Interval<Instant>> ComputeCollisionIntervals(
  ...
  // `reference` on either side; the extremities are clamped where `reference`
  // can still be evaluated, as `ComputeApsides` clamps the apsides.
  Instant const t_max = std::min(trajectory.t_max(), reference.t_max());
  Instant const t_min = std::max(trajectory.t_min(), reference.t_min());
  if (t_max < t_min) {
    return {};
  }
  absl::btree_set<Instant> apsides_times{t_min, t_max};
